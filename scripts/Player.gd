...
func check_vert_collision(wall_from, circle_pos, circle_radius):
	...
	if circle_pos.y < wall_from.y:
		return circle_radius > sqrt(dx_pow + pow(circle_pos.y - wall_from.y, 2))
	if circle_pos.y <= wall_to.y:
		return circle_radius > abs(circle_pos.x - wall_from.x)
...
func check_hor_collision(wall_from, circle_pos, circle_radius):
	...
	if circle_pos.x < wall_from.x:
		return circle_radius > sqrt(dy_pow + pow(circle_pos.x - wall_from.x, 2))
	if circle_pos.x <= wall_to.x:
		return circle_radius > abs(circle_pos.y - wall_from.y)
...
func is_collide(pos):
	...
	for i in range(4):
		...
		if is_vert:
			if check_vert_collision(wall_from, pos, 0.2):
				return true
		else:
			if check_hor_collision(wall_from, pos, 0.2):
				return true
	return false
...
func _process(delta):
	...
	if velocity.length() > 0:
		...
		if not is_collide(new_pos_x / 64):
			position.x = new_pos.x
		var new_pos_y = Vector2(position.x, new_pos.y)
		if not is_collide(new_pos_y / 64):
			position.y = new_pos.y


func _draw():
	draw_circle(Vector2(), 16, Color(0.5, 0, 0))
